"""Unit tests for KlaviyoClient adapter."""

from unittest.mock import MagicMock, patch

import httpx
import pytest

from app.clients.klaviyo_client import KlaviyoClient, KlaviyoClientError
from app.core.config import Settings


@pytest.fixture
def klaviyo_settings() -> Settings:
    """Return settings with a Klaviyo API key configured."""
    return Settings(
        KLAVIYO_API_KEY="pk_test_klaviyo_key",
        KLAVIYO_MAX_RETRIES=3,
        FRONTEND_RESET_URL="http://localhost:3000/reset-password",
    )


def test_send_password_reset_email_success(klaviyo_settings: Settings) -> None:
    """Successful Klaviyo API call should complete without error."""
    client = KlaviyoClient(klaviyo_settings)
    mock_response = MagicMock()
    mock_response.status_code = 202

    with patch("app.clients.klaviyo_client.httpx.Client") as mock_client_cls:
        mock_client = MagicMock()
        mock_client.__enter__.return_value = mock_client
        mock_client.post.return_value = mock_response
        mock_client_cls.return_value = mock_client

        client.send_password_reset_email("user@example.com", "reset-token-123")

    mock_client.post.assert_called_once()


def test_send_password_reset_email_retries_on_500(klaviyo_settings: Settings) -> None:
    """500 responses should be retried up to max_retries."""
    client = KlaviyoClient(klaviyo_settings)
    fail_response = MagicMock()
    fail_response.status_code = 500
    success_response = MagicMock()
    success_response.status_code = 202

    with patch("app.clients.klaviyo_client.httpx.Client") as mock_client_cls:
        mock_client = MagicMock()
        mock_client.__enter__.return_value = mock_client
        mock_client.post.side_effect = [fail_response, success_response]
        mock_client_cls.return_value = mock_client

        with patch("app.clients.klaviyo_client.time.sleep"):
            client.send_password_reset_email("user@example.com", "reset-token-123")

    assert mock_client.post.call_count == 2


def test_send_password_reset_email_request_error_retries(
    klaviyo_settings: Settings,
) -> None:
    """Network errors should be retried."""
    client = KlaviyoClient(klaviyo_settings)
    success_response = MagicMock()
    success_response.status_code = 202

    with patch("app.clients.klaviyo_client.httpx.Client") as mock_client_cls:
        mock_client = MagicMock()
        mock_client.__enter__.return_value = mock_client
        mock_client.post.side_effect = [
            httpx.RequestError("timeout"),
            success_response,
        ]
        mock_client_cls.return_value = mock_client

        with patch("app.clients.klaviyo_client.time.sleep"):
            client.send_password_reset_email("user@example.com", "reset-token-123")

    assert mock_client.post.call_count == 2


def test_send_password_reset_email_failure_after_retries(
    klaviyo_settings: Settings,
) -> None:
    """Persistent failures should raise KlaviyoClientError."""
    client = KlaviyoClient(klaviyo_settings)
    fail_response = MagicMock()
    fail_response.status_code = 500

    with patch("app.clients.klaviyo_client.httpx.Client") as mock_client_cls:
        mock_client = MagicMock()
        mock_client.__enter__.return_value = mock_client
        mock_client.post.return_value = fail_response
        mock_client_cls.return_value = mock_client

        with patch("app.clients.klaviyo_client.time.sleep"):
            with pytest.raises(KlaviyoClientError):
                client.send_password_reset_email("user@example.com", "reset-token-123")


def test_send_password_reset_email_missing_api_key() -> None:
    """Missing API key should raise KlaviyoClientError."""
    client = KlaviyoClient(Settings(KLAVIYO_API_KEY=""))
    with pytest.raises(KlaviyoClientError):
        client.send_password_reset_email("user@example.com", "reset-token-123")
