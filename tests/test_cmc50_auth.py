...
def test_cmc50_forgot_password_inactive_user_no_klaviyo_call(
    ...
    """CMC-50: Inactive user forgot-password returns generic 200, no email sent."""
...
def test_cmc50_login_invalid_password_401(
    ...
    """CMC-50: Invalid credentials return 401."""
    response = db_client.post(
        LOGIN_URL,
...
def test_cmc50_login_unknown_user_401(
    db_client: TestClient,
    mock_klaviyo_client,
) -> None:
...
def test_cmc50_login_empty_password_422(
    db_client: TestClient,
    mock_klaviyo_client,
) -> None:
...
def test_cmc50_login_unicode_username(
    ...
    """Edge case: unicode username login succeeds when user exists."""
    user = User(
...
def test_cmc50_access_token_has_access_type(
    db_client: TestClient,
    admin_user,
    mock_klaviyo_client,
) -> None:
    """Auth: access token payload type is access."""
    response = db_client.post(
        LOGIN_URL,
        json={"email_or_username": "admin@test.com", "password": "TestAdmin123!"},
    )
...
def test_cmc50_refresh_token_has_refresh_type(
    db_client: TestClient,
    admin_user,
    mock_klaviyo_client,
) -> None:
    """Auth: refresh token payload type is refresh."""
    response = db_client.post(
        LOGIN_URL,
        json={"email_or_username": "admin@test.com", "password": "TestAdmin123!"},
    )
...
def test_cmc50_expired_token_rejected(expired_access_token) -> None:
    """Auth: expired JWT is rejected on decode."""
    with pytest.raises(JWTError):
        decode_token(expired_access_token, get_settings())


def test_cmc50_invalid_token_rejected() -> None:
    """Auth: malformed JWT string is rejected on decode."""
    with pytest.raises(JWTError):
